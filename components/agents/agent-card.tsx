...
const statusConfig = {
  ...
  idle: { label: "Idle", className: "bg-yellow-500/20 text-yellow-400 border-yellow-500/30", dot: "bg-yellow-400" },
  error: {
    label: "Error",
...
export function AgentCard({
  ...
  const agentSlug = agentSlugMap[type] || "custom-agent"

  return (
    <Link href={`/dashboard/agents/${agentSlug}?id=${id}`}>
      <Card className="bg-card border-border hover:border-primary/30 transition-colors cursor-pointer">
        ...
        <CardFooter className="flex items-center justify-between pt-3 border-t border-border">
          ...
          <Button size="sm" variant="ghost" className="h-8 gap-1.5" onClick={(e) => e.preventDefault()}>
            {status === "running" ? (
              <>
